...
class Runner:
  ...
  def __init__(self,
    ...
    self.logger.info(
      f'{self.run_name} - train_x shape: {self.train_x.shape}')
    self.logger.info(
      f'{self.run_name} - train_y shape: {self.train_y.shape}')
  ...
  def visualize_corr(self):
    ...
    df[self.traget] = self.train_y.copy()
    ...
    # 保存
    plt.savefig(self.out_dir_name + self.run_name +
          '_corr.png', dpi=300, bbox_inches="tight")
    plt.close()
  ...
  def shap_feature_importance(self) -> None:
    ...
    ax.barh(ma_shap.index, ma_shap['Mean Abs Shapley'],
        label='Mean Abs Shapley',  align="center", alpha=0.8)
    labels = ax.get_xticklabels()
    plt.setp(labels, rotation=0, fontsize=10)
    ax.legend(loc='upper left')
    plt.savefig(self.out_dir_name + self.run_name +
          '_shap.png', dpi=300, bbox_inches="tight")
    plt.close()
    ...
    def train_fold(self, i_fold: Union[int, str]) -> Tuple[
      ...
      if validation:
        ...
        if self.calc_shap:
          va_pred, self.shap_values[va_idx[:shap_sampling]] = model.predict_and_shap(
            va_x, shap_sampling)
        else:
    ...
    def run_train_cv(self) -> None:
      ...
      if self.cv_method == 'KFold':
        self.logger.info(
          f'{self.run_name} - cv method: {self.cv_method}')
      else:
        self.logger.info(
          f'{self.run_name} - cv method: {self.cv_method} - target: {self.cv_target_column}')
      ...
      for i_fold in range(self.n_splits):
        # 学習を行う
        self.logger.info(
          f'{self.run_name} fold {i_fold} - start training')
      ...
      if self.save_train_pred:
        Util.dump_df_pickle(pd.DataFrame(
          preds), self.out_dir_name + f'.{self.run_name}-train.pkl')
    ...
    def run_predict_cv(self) -> None:
      ...
      for i_fold in range(self.n_splits):
        ...
        preds.append(pred)
        self.logger.info(
          f'{self.run_name} - end prediction fold:{i_fold}')

      # 予測の平均値を出力する
      pred_avg = (np.mean(preds, axis=0) > 0.5).astype(int)

      # 推論結果の保存（submit対象データ）
      Util.dump_df_pickle(pd.DataFrame(pred_avg),
                self.out_dir_name + f'{self.run_name}-pred.pkl')
    ...
    def load_x_train(self) -> pd.DataFrame:
      ...
      """
      # 学習データの読込を行う
      dfs = [pd.read_pickle(self.feature_dir_name +
                  f'{f}_train.pkl') for f in self.features]
    ...
    def load_y_train(self) -> pd.Series:
      ...
      train_y = pd.read_pickle(
        self.feature_dir_name + self.target + '_train.pkl')
    ...
    def load_x_test(self) -> pd.DataFrame:
      ...
      """
      dfs = [pd.read_pickle(self.feature_dir_name +
                  f'{f}_test.pkl') for f in self.features]
    ...
    def load_stratify_or_group_target(self) -> pd.Series:
      ...
      """
      df = pd.read_pickle(self.feature_dir_name +
                self.cv_target_column + '_train.pkl')
  ...
  def train_fold(self, i_fold: Union[str, int]) -> Tuple[
    ...
    if validation:
      ...
      tr_data = self.dataset.get_data(
        [i for i in range(2008, 2018)], 'tokyo')
      tr_Y = tr_data['target']
  ...
  def get_predict_cv(self):
    ...
    for i_fold in range(self.n_fold):
      ...
      scores.append(score)
      logger.info(
        f'{self.run_name} - end prediction fold:{i_fold} - score {score}')
