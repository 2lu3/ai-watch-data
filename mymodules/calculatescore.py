...
class CalculateF1Score:
  def __init__(self, Y, y_pred, class_num, verbose = False):
    ...
    self.class_true_positives = [np.sum(self.class_true_positive(i)) for i in range(class_num)]
    if verbose == True:
  ...
  def class_true_positive(self, class_label):
    return np.logical_and(self.y_true_normalized == class_label,
            self.y_pred_normalized == class_label)

  def class_precision(self, class_label):
    return self.class_true_positives[class_label] / (np.sum(self.y_pred_normalized == class_label) + self.epsilon)

  def class_recall(self, class_label):
    return self.class_true_positives[class_label] / (np.sum(self.y_true_normalized == class_label) + self.epsilon)
  ...
  def macro_precision(self):
    return sum([self.class_precision(i) for i in range(self.class_num)]) / (self.class_num + self.epsilon)
